#######################################################################################
### Run harp seal lgcp model using inlabru and including heirarchical ice covariate ###
#######################################################################################
...
# need to load in the point data - stick with subset or try more points...?!
...
##############################################
### Put this all together in an lgcp model ###
##############################################

# could drop the AR1 process if it speeds up model fitting at the testing phase...
...
m1 <- coordinates ~ myIceSingle(f.ice_single(x, y), model = "linear") + Intercept(1)
m2 <- coordinates ~ myIceSeasonal(f.ice_seasonal(x, y, season), model = "linear") + Intercept(1)
m3 <- coordinates ~ myIceResidual(f.ice_residual(x, y, season, year_i), model = "linear") + Intercept(1)

m4 <- coordinates ~ myIceSingle(f.ice_single(x, y), model = "linear") + 
                    myIceSeasonal(f.ice_seasonal(x, y, season), model = "linear") + 
                    myIceResidual(f.ice_residual(x, y, season, year_i), model = "linear") + Intercept(1)
...
ice_mesh <- inla.mesh.1d(loc = seq(from = -100, to = 100, by = 50), boundary = "free")
ice_spde <- inla.spde2.pcmatern(ice_mesh, prior.range = c(5, 0.01), prior.sigma = c(3, 0.01)) 

# model formula
m2 <- coordinates ~ myIce(f.ice_single(x, y), model = "rw2", mapper = bru_mapper(ice_mesh, indexed = F)) + Intercept(1)
